...
function sponsorSummaryRows(rows) {
  const activeCount = rows.filter(row => row.status === 'active').length;
  const cashCommitted = rows.reduce((sum, row) => sum + Number(row.cashCommitted || 0), 0);
  const cashReceived = rows.reduce((sum, row) => sum + Number(row.cashReceived || 0), 0);
  const inKindValue = rows.reduce((sum, row) => sum + Number(row.inKindValue || 0), 0);
...
function renderSponsorSection({ force = false } = {}) {
  if (route() !== 'economics') return;

  const content = document.querySelector('#economics-section-content');
  if (!content) return;
  if (!force && content.querySelector('[data-economics-sponsor-root]')) return;

  const year = selectedYear();
  const rows = sponsorsForYear(year);
  const summary = sponsorSummaryRows(rows);
  ...
  content.innerHTML = `
    <section data-economics-sponsor-root>
      <section class="economics-subhead economics-sponsor-head">
        ...
        ${writable ? '<button class="button button-primary" id="economics-add-sponsor" type="button">+ Nuovo sponsor</button>' : ''}
      </section>
      ...
      <div class="economics-sponsor-accounting-note">
        Il cash ricevuto dagli sponsor viene incluso automaticamente nei KPI di sostenibilità dell’Overview. Non duplicarlo come entrata nei Pagamenti.
      </div>

      <section class="economics-kpis economics-sponsor-kpis">
        <article class="economics-kpi"><span>Sponsor attivi</span><strong>${summary.activeCount}</strong></article>
        <article class="economics-kpi"><span>Cash concordato</span><strong>${money(summary.cashCommitted)}</strong></article>
        <article class="economics-kpi"><span>Cash ricevuto</span><strong>${money(summary.cashReceived)}</strong></article>
        <article class="economics-kpi"><span>Valore in-kind</span><strong>${money(summary.inKindValue)}</strong></article>
        <article class="economics-kpi"><span>Valore acquisito</span><strong>${money(summary.totalValue)}</strong></article>
      </section>

      ${rows.length ? `
        <div class="economics-sponsor-grid">
          ${rows.map(sponsor => sponsorCard(sponsor, writable)).join('')}
        </div>
      ` : `
  ...
  return `
    <article class="panel economics-sponsor-card">
      <div class="panel-body">
        <div class="economics-sponsor-card-head">
          <div>
            <div class="economics-sponsor-badges">
              <span class="economics-sponsor-status status-${escapeAttr(sponsor.status || 'prospect')}">${escapeHtml(statusLabels[sponsor.status] || 'Prospect')}</span>
              <span class="economics-sponsor-type">${escapeHtml(typeLabels[sponsor.type] || 'Economico')}</span>
            </div>
            <h3>${escapeHtml(sponsor.name || 'Sponsor')}</h3>
            <p>${escapeHtml(formatPeriod(sponsor))}</p>
          </div>
          ${writable ? `<button class="icon-button" type="button" data-edit-sponsor="${escapeAttr(sponsor.id)}" aria-label="Modifica sponsor">✎</button>` : ''}
        </div>
        ...
        ${contact ? `<div class="economics-sponsor-contact"><strong>Contatto</strong><span>${escapeHtml(contact)}</span></div>` : ''}
        ${sponsor.benefits ? `<div class="economics-sponsor-note"><strong>Fornitura / benefit</strong><span>${escapeHtml(sponsor.benefits)}</span></div>` : ''}
        ${sponsor.deliverables ? `<div class="economics-sponsor-note"><strong>Impegni atleta / visibilità</strong><span>${escapeHtml(sponsor.deliverables)}</span></div>` : ''}
        ${sponsor.notes ? `<div class="economics-sponsor-note"><strong>Note</strong><span>${escapeHtml(sponsor.notes)}</span></div>` : ''}
      </div>
    </article>
  `;
}
...
function openSponsorDialog(sponsorId = '') {
  ...
  dialog.innerHTML = `
    <form method="dialog" id="economics-sponsor-form">
      ...
      <div class="dialog-body form-grid">
        <div class="field full"><label>Nome / azienda</label><input name="name" required value="${escapeAttr(sponsor.name)}" /></div>
        <div class="field"><label>Stagione</label><input name="seasonYear" type="number" min="2000" max="2100" required value="${escapeAttr(sponsor.seasonYear || selectedYear())}" /></div>
        <div class="field"><label>Stato</label><select name="status">${statusOptions(sponsor.status)}</select></div>
        <div class="field"><label>Tipo accordo</label><select name="type">${typeOptions(sponsor.type)}</select></div>
        <div class="field"><label>Sito</label><input name="website" type="url" value="${escapeAttr(sponsor.website)}" placeholder="https://…" /></div>

        <div class="field"><label>Referente</label><input name="contactName" value="${escapeAttr(sponsor.contactName)}" /></div>
        <div class="field"><label>Email</label><input name="contactEmail" type="email" value="${escapeAttr(sponsor.contactEmail)}" /></div>
        <div class="field"><label>Telefono</label><input name="contactPhone" value="${escapeAttr(sponsor.contactPhone)}" /></div>
        <div class="field"><label>Data inizio</label><input name="startDate" type="date" value="${escapeAttr(sponsor.startDate)}" /></div>
        <div class="field"><label>Data fine</label><input name="endDate" type="date" value="${escapeAttr(sponsor.endDate)}" /></div>

        <div class="field"><label>Cash concordato (€)</label><input name="cashCommitted" type="number" min="0" step="0.01" value="${escapeAttr(sponsor.cashCommitted)}" /></div>
        <div class="field"><label>Cash ricevuto (€)</label><input name="cashReceived" type="number" min="0" step="0.01" value="${escapeAttr(sponsor.cashReceived)}" /></div>
        <div class="field"><label>Valore in-kind (€)</label><input name="inKindValue" type="number" min="0" step="0.01" value="${escapeAttr(sponsor.inKindValue)}" /></div>
        <div class="field full"><label>Fornitura / benefit</label><textarea name="benefits" placeholder="Materiale, servizi, viaggi, abbigliamento…">${escapeHtml(sponsor.benefits)}</textarea></div>
        <div class="field full"><label>Impegni atleta / visibilità</label><textarea name="deliverables" placeholder="Logo, social, eventi, contenuti, presenza…">${escapeHtml(sponsor.deliverables)}</textarea></div>
        <div class="field full"><label>Note</label><textarea name="notes">${escapeHtml(sponsor.notes)}</textarea></div>
      </div>

      <div class="dialog-actions">
        <div>${current ? '<button class="button button-danger-ghost" id="economics-delete-sponsor" type="button">Elimina</button>' : ''}</div>
  ...
  dialog.querySelector('#economics-sponsor-form')?.addEventListener('submit', event => {
    ...
    store.update(state => {
      ...
      const index = state.economics.sponsors.findIndex(item => item.id === normalized.id);
      if (index >= 0) state.economics.sponsors[index] = normalized;
      else state.economics.sponsors.push(normalized);
    });

    dialog.close();
    renderSponsorSection({ force: true });
  });
  ...
  dialog.querySelector('#economics-delete-sponsor')?.addEventListener('click', async () => {
    const confirmed = await showInAppConfirm(
      ...
      { title: 'Elimina sponsor', confirmLabel: 'Elimina', danger: true },
    );
    ...
    store.update(state => {
      ...
      state.economics.sponsors = state.economics.sponsors.filter(item => item.id !== sponsor.id);
    });
...
function applyEconomicsReadOnly() {
  ...
  for (const selector of mutationSelectors) {
    main.querySelectorAll(selector).forEach(element => {
      element.hidden = !writable || (sponsorModeActive && selector === '#economics-add-entry');
    });
  }
  ...
  if (!writable && !banner) {
    ...
    if (head) {
      ...
      banner.textContent = 'Economics in sola lettura: puoi consultare costi, budget e sponsor ma non modificarli.';
...
function enhanceOverviewWithSponsors() {
  ...
  const main = document.querySelector('#main-content');
  const overviewActive = main?.querySelector('[data-economics-section="overview"].active');
  if (!main || !overviewActive) return;

  const year = selectedYear();
  const economics = store.getState().economics || {};
  const entries = (Array.isArray(economics.entries) ? economics.entries : []).filter(entry => {
  ...
  for (const kpi of main.querySelectorAll('.economics-kpi')) {
    ...
    if (label.textContent.trim().startsWith('Contributi / rimborsi')) {
      label.textContent = 'Contributi / rimborsi + sponsor';
      value.textContent = money(totalCredits);
    } else if (label.textContent.trim() === 'Costo netto pagato') {
      value.textContent = money(paid - totalCredits);
    }
  }
}
...
function queueEnhancement() {
  ...
  window.queueMicrotask(enhanceEconomics);
}

document.addEventListener('click', event => {
  if (route() !== 'economics') return;

  const nativeSectionButton = event.target.closest?.('[data-economics-section]');
